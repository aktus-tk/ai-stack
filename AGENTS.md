...
タスクの規模・複雑性・リスクに応じて実行方法を選択する。

- 軽微な調査や変更は、担当エージェントが直接処理する。
- 作業の分割や並行化に明確な効果がある場合のみ、サブエージェントへ委譲する。
- 独立したレビューは、影響範囲や回帰リスクが大きい変更に対して実施する。
- 委譲・調整コストが作業そのものを上回る場合、サブエージェントを使用しない。
...
クラウド CLI (awscli / tccli / gcloud / az / coscli / tcclit / gcloudt / awst 等)・GitHub CLI (gh) のインストール・`command not found` 対応は、`install-cli-tools` Skill (`skills/install-cli-tools/SKILL.md`) を使用する。対象: plgl, iron, dmdb, choco, visualive, visualize。
