...
**基本は director（メインエージェント）単独で実行する。**

- ユーザーが明示的に委譲を指示した場合のみ、サブエージェント (engineer / qa / explore) を使用する。
- サブエージェントの利用が有効と判断した場合も、実行前にユーザーへ確認する。
- 軽微な調査・変更・レビューはすべて director が直接処理する。
