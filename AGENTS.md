...
8. **同一原因による失敗の再試行は最大2回まで**とする。
9. 再試行は、新しい観測・仮説・設定変更など、**前回から有意な変化がある場合のみ**行う。同じ操作を根拠なく繰り返さない。
10. 2回失敗した場合は作業を停止し、確認した事実・試した内容・推定原因・続行に必要な情報を報告してユーザーの判断を仰ぐ。
11. **ユーザーが具体的なコマンド・手順を指定した場合、それを再調査・代替・事前検証せず実行する。** 安全上の明確な問題がある場合のみ停止する。
