...
2. **harness-mem daemon への疎通確認**
   ```bash
   curl -s http://100.92.131.75:37888/health
   ```

3. **クライアントセットアップ** → `runbooks/bootstrap-client.md` を実行

## セキュリティ

- harness-mem daemon / Caddy への到達は Tailscale メッシュに依存。
  サーバーへのアクセスは Tailscale ACL (`tailscale` admin console) で制御する。
  現行 ACL (`autogroup:member` → `100.92.131.75`) は
  `tcp:37888`, `tcp:37889`, `tcp:8090` を許可している (`skills/tailscale/SKILL.md` 参照)。
